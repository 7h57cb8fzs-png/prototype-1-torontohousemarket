...
const fixture = () => ({generated_at:'2026-09-06T00:00:00Z', facts:{listing_key:'N1000001',for_sale:true,list_price:1000000,property_type:'Condo Townhouse',living_area:'1000-1199',neighbourhood:'Test Community',beds:2,baths:2,annual_tax:4800,tax_year:2026,maintenance_fee:{amount:400,frequency:'month',included:['Water']}},valuation:{available:true,low:900000,midpoint:1000000,high:1100000,confidence:'Medium'},comparable_policy:{windowDays:100},comparables:[1,2,3,4,5].map(n=>({listingKey:`N100000${n+1}`,address:`${n} Test Street`,soldPrice:900000+n*10000,soldDate:'2026-08-15',cityRegion:'Test Community',propertySubType:'Condo Townhouse',livingAreaRange:'1000-1199'})),narrative:{executive_summary:'Strong value',questions_for_realtor:['What are the planned building repairs?']},ai_generation:{provider:'cloudflare'}});
test('null, zero and unavailable asking prices cannot become bargain ratings',()=>{
 for (const price of [null,undefined,0,-1,'']) {const f=fixture();f.facts.list_price=price;const e=propertyReportEmail('Test', {display_name:'Unassigned'},f);assert.ok(!/Strong value|\/10|below the evidence band/.test(e.html));assert.match(e.html,/Golestan Homes/);assert.ok(!e.html.includes('Unassigned'));}
 const f=fixture();f.facts.for_sale=false;const e=propertyReportEmail('Test',{},f);assert.ok(!e.html.includes('Request a showing'));assert.match(e.html,/Call Golestan Homes/);
});
