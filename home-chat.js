...
const FILTER_SCHEMA=object({cities:{type:'array',items:{type:'string',enum:CITIES}},type:{type:'string',enum:TYPES},types:{type:'array',items:{type:'string',enum:TYPES.filter(t=>t!=='any')}},mode:{type:'string',enum:MODES},minPrice:num,maxPrice:{type:['integer','null']},minBeds:num,maxBeds:num,minBaths:num,minParking:num,minSqft:num,area:str,brokerage:str,sort:{type:'string',enum:SORTS},checks:{type:'array',items:str}});
...
const DEFAULT={cities:['Toronto'],type:'any',types:[],mode:'all',minPrice:0,maxPrice:null,minBeds:0,maxBeds:0,minBaths:0,minParking:0,minSqft:0,area:'',brokerage:'',sort:'newest',checks:[]};
const within=(n,a,b)=>Number.isInteger(n)&&n>=a&&n<=b;
export function validChatFilters(f){return f&&Array.isArray(f.cities)&&f.cities.length>0&&f.cities.length<=3&&new Set(f.cities).size===f.cities.length&&f.cities.every(c=>CITIES.includes(c))&&TYPES.includes(f.type)&&(f.types===undefined||Array.isArray(f.types)&&f.types.length<=4&&f.types.every(t=>TYPES.includes(t)&&t!=='any'))&&MODES.includes(f.mode)&&SORTS.includes(f.sort)&&within(f.minPrice,0,20000000)&&(f.maxPrice===null||within(f.maxPrice,100000,20000000))&&(!f.maxPrice||f.minPrice<=f.maxPrice)&&['minBeds','maxBeds','minBaths','minParking'].every(k=>within(f[k],0,9))&&(!f.maxBeds||f.maxBeds>=f.minBeds)&&within(f.minSqft,0,20000)&&typeof f.area==='string'&&f.area.length<=80&&(f.brokerage===undefined||typeof f.brokerage==='string'&&f.brokerage.length<=100)&&Array.isArray(f.checks)&&f.checks.length<=5&&f.checks.every(x=>typeof x==='string'&&x.length<=140);}
...
async function searchHomes(filters,request,env,ctx,app){
  const data=await Promise.all(filters.cities.map(async city=>{
    const u=new URL('/api/discovery',request.url);for(const [k,v]of Object.entries({...filters,city,limit:60,query:true}))if(!['checks','cities'].includes(k)&&v!==null&&v!==undefined)u.searchParams.set(k,String(v));
...
async function propertyHome(query,request,env,ctx,app){
  const mls=query.match(/\b[A-Z]\d{7,9}\b/i);if(mls)query=mls[0].toUpperCase();
...
Schools, transit distance, lifestyle, walkability, condition, pools, yards, investment returns cannot be verified with these fields; preserve those requests in checks, explain that they need verification, and still search the verifiable filters. For rentals, explain this finder currently covers homes for sale; don't return sale results as rentals. With no prior context, do not search a default city without naming it in a clarification. When greeting, ask which city/budget/type. clarification is only a concise user-facing question/message for clarify; otherwise empty.`,
    {message:query,previousFilters:previous?.filters||DEFAULT,hasPreviousSearch:!!previous?.searched,conversation:previous?.messages||[],previousHomes:previous?.lastShown||previous?.recent||[],supportedCities:CITIES},1300);
  ...
  const explicitKeys=[...query.matchAll(/\b[A-Z]\d{7,9}\b/gi)];
  if(explicitKeys.length===1&&/tell me about|show me|check|view/i.test(query))plan.action='property';
  ...
  if(plan.action==='search'){
    ...
    const explicitMls=query.match(/\b[A-Z]\d{7,9}\b/i)?.[0]?.toUpperCase();
    const known=explicitMls&&(previous?.recent||[]).find(h=>h.listingKey===explicitMls);
    const result=known&&Date.now()-Date.parse(previous?.checkedAt||0)<300000?{homes:[known],note:'Listing facts from the current search.'}:await propertyHome(explicitMls||plan.propertyQuery,request,env,ctx,app);pool=result.homes;offset=0;note=result.note;coverage={partial:false,matched:pool.length,scanned:pool.length};checkedAt=new Date().toISOString();
  ...
  const reply=answer.reply;let followups=(answer.followups||[]).filter(x=>typeof x==='string'&&x.length<=100).slice(0,3);
  if(plan.action==='search'&&!homes.length){const place=filters.area||filters.cities.join(' and ');if(filters.brokerage)followups=[`Show ${place} homes from all brokerages`,...followups];else if(filters.area)followups=[`Search across ${filters.cities.join(' and ')}, any neighbourhood`,...followups];followups=[...new Set(followups)].slice(0,3);}
  const state=await signChatState({filters,pool,offset,coverage,checkedAt,recent,lastShown,searched:previous?.searched||['search','property'].includes(plan.action),messages:[...messages,{role:'user',content:query},{role:'assistant',content:reply}].slice(-12)},env);
