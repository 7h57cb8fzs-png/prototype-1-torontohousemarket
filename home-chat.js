...
export function conciseReply(value){let result='';for(const sentence of String(value||'').trim().split(/(?<=[.!?])\s+/)){if(/(?:more|additional|further) (?:results|homes|listings|options).*(?:remain|available|left)|no (?:additional|further) (?:results|homes|listings|options)/i.test(sentence))continue;if(result&&(result+' '+sentence).split(/\s+/).length>75)break;result+=(result?' ':'')+sentence;}return result;}
...
Schools, transit distance, lifestyle, walkability, condition, pools, yards, investment returns cannot be verified with these fields; preserve those requests in checks, explain that they need verification, and still search the verifiable filters. For rentals, explain this finder currently covers homes for sale; don't return sale results as rentals. With no prior context, do not search a default city without naming it in a clarification. When greeting, ask which city/budget/type. clarification is only a concise user-facing question/message for clarify; otherwise empty.`,
  ...
  try{
    answer=await modelJSON(env,'thm_home_conversation',REPLY_SCHEMA,
      ...
      {message:query,action:plan.action,filters,currentHomes:plan.action==='answer'?lastShown:homes,previousHomes:lastShown,conversation:messages,coverage,note},1000);
  ...
  const reply=conciseReply(answer.reply)||(homes.length?'These homes match your search. Which would you like to explore?':'Try changing the area, budget or home type to see another set of homes.');let followups=(answer.followups||[]).filter(x=>typeof x==='string'&&x.length<=100).slice(0,3);
