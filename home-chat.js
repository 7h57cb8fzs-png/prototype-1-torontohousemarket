...
const FILTER_SCHEMA=object({cities:{type:'array',items:{type:'string',enum:CITIES}},type:{type:'string',enum:TYPES},mode:{type:'string',enum:MODES},minPrice:num,maxPrice:{type:['integer','null']},minBeds:num,maxBeds:num,minBaths:num,minParking:num,minSqft:num,area:str,brokerage:str,sort:{type:'string',enum:SORTS},checks:{type:'array',items:str}});
...
const DEFAULT={cities:['Toronto'],type:'any',mode:'all',minPrice:0,maxPrice:null,minBeds:0,maxBeds:0,minBaths:0,minParking:0,minSqft:0,area:'',brokerage:'',sort:'newest',checks:[]};
const within=(n,a,b)=>Number.isInteger(n)&&n>=a&&n<=b;
export function validChatFilters(f){return f&&Array.isArray(f.cities)&&f.cities.length>0&&f.cities.length<=3&&new Set(f.cities).size===f.cities.length&&f.cities.every(c=>CITIES.includes(c))&&TYPES.includes(f.type)&&MODES.includes(f.mode)&&SORTS.includes(f.sort)&&within(f.minPrice,0,20000000)&&(f.maxPrice===null||within(f.maxPrice,100000,20000000))&&(!f.maxPrice||f.minPrice<=f.maxPrice)&&['minBeds','maxBeds','minBaths','minParking'].every(k=>within(f[k],0,9))&&(!f.maxBeds||f.maxBeds>=f.minBeds)&&within(f.minSqft,0,20000)&&typeof f.area==='string'&&f.area.length<=80&&(f.brokerage===undefined||typeof f.brokerage==='string'&&f.brokerage.length<=100)&&Array.isArray(f.checks)&&f.checks.length<=5&&f.checks.every(x=>typeof x==='string'&&x.length<=140);}
