...
export function basicSearch(q,city='Toronto'){
  ...
  return {city:found[0]||(district?'Toronto':city),type,mode:/just listed|new listing/.test(text)?'new':/price (?:drop|reduc)/.test(text)?'reduced':/luxury|2m\+/.test(text)?'luxury':'all',maxPrice:amount,minBeds:beds?+beds[1]:0,minBaths:0,minParking:/parking/.test(text)?1:0,area:district||'',checks:[],needsModel:found.length>1||!found.length&&!district||/school|transit|subway|yard|pool|family|walk|near|between|at least|bath/.test(text)};
}
...
export async function homeSearch(request,env,ctx,app){
  ...
  if(q){const result=await interpret(q,env,basicSearch(q,u.searchParams.get('city')||'Toronto'));filters=result.filters;mode=result.mode;}
  ...
  const summary=[filters.city,filters.area,filters.type==='any'?'':filters.type.replaceAll('_',' '),filters.minBeds?`${filters.minBeds}+ bedrooms`:'',filters.maxPrice?`up to $${filters.maxPrice.toLocaleString('en-CA')}`:'',filters.minParking?`${filters.minParking}+ parking`:''].filter(Boolean).join(' · ');
