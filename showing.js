...
if(!token){$('appointmentAddress').textContent='Open the showing link in your AI report email.';$('appointmentError').textContent='Use the call button below to arrange your showing.';}else api('/api/appointments').then(render).catch(e=>{$('appointmentAddress').textContent='Let’s arrange your next step.';$('appointmentError').textContent=e.message;});
