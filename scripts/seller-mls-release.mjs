...
async function verify(base){
 const r=await fetch(base+'/api/version',{cache:'no-store',signal:AbortSignal.timeout(20000)});assert(r.ok);const d=await r.json();assert.equal(d.seller_input,'address_and_historical_mls_only');
 ...
 for(const path of ['/seller-mls-input.js','/seller-archive.js','/worker-v22.js','/wrangler.seller-mls-release.json']){const res=await fetch(base+path,{signal:AbortSignal.timeout(10000)});const body=await res.text();assert(!body.includes('historicalSellerProperty')&&!body.includes('SUPABASE_SERVICE_ROLE_KEY')&&!body.includes('"vars"'),'Server-only content exposed');}
