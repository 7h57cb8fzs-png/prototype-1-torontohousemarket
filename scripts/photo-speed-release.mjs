...
if(process.env.EXPECTED_CANDIDATE_SHA)assert.equal(source(after),process.env.EXPECTED_CANDIDATE_SHA,'Photo source differs from reviewed preview');
...
const listings=['C13813214','W13812424','N13816518'];
const baseline = new Map();
async function property(base,key,deferred=false,probe=false){
 const started=Date.now();
 const r=await fetch(`${base}/api/property?listingKey=${key}${deferred?'&defer_photos=1':''}${probe?'&photo_probe='+process.env.GITHUB_SHA:''}`,{signal:AbortSignal.timeout(45000)});
 assert(r.ok,`Property ${key}: HTTP ${r.status}`);
 const d=await r.json();assert(d.ok&&d.property?.listingKey===key,'Wrong property: '+key);
 return {data:d.property,milliseconds:Date.now()-started};
}
const identity=p=>(p.photos||[]).map(v=>({key:v.key,primary:v.primary,sequence:v.sequence}));
const facts=p=>Object.fromEntries(['listingKey','address','listPrice','forSale','forLease','beds','baths','propertySubType','livingAreaRange'].map(k=>[k,p[k]]));
async function verify(base){
 ...
 for(const file of ['index.html','app.js','styles.css','seller.html','seller.js','seller.css','address-input.js','interface.css']){
  const r=await fetch(base+'/'+(file==='index.html'?'':file)+'?photo_speed='+process.env.GITHUB_SHA);
  assert(r.ok&&hash(Buffer.from(await r.arrayBuffer()))===hash(readFileSync(file)),'Asset mismatch: '+file);
 }
 const results=[];
 for(const key of listings){
  if(!baseline.has(key)) baseline.set(key,await property(`https://${prior.slice(0,8)}-${worker}.7h57cb8fzs.workers.dev`,key,false,true));
  const fast=await property(base,key,true,true),gallery=await property(base,key);
  assert.deepEqual(facts(fast.data),facts(gallery.data),'Facts differ with deferred gallery: '+key);
  assert.deepEqual(identity(gallery.data),identity(baseline.get(key).data),'MLS cover/order/count changed: '+key);
  assert.equal(fast.data.photosPending,true);assert.equal(fast.data.photos.length,0);
  ...
  if(process.env.PUBLISH!=='true'){
   const sizes={};
   for(const [size,image] of Object.entries({full:cover,mobile:cover.mobile,thumbnail:cover.thumbnail})){
    const r=await fetch(image.url,{signal:AbortSignal.timeout(30000)});assert(r.ok,'Image failed: '+key+' '+size);assert(r.headers.get('content-type')?.startsWith('image/'));
    sizes[size]=(await r.arrayBuffer()).byteLength;
   }
   result.imageBytes=sizes;
   assert(sizes.mobile<sizes.full&&sizes.thumbnail<sizes.full,'Small variants should reduce transferred bytes');
  }
  results.push(result);console.log(JSON.stringify({stage:'photo-performance',...result}));
 }
 console.log(JSON.stringify({stage:'verified',url:base,assets:8,listings:3}));
 return results;
}
await verify(preview);
if(process.env.PUBLISH!=='true'){
 const {verifyPhotoBrowser}=await import('./photo-speed-browser.mjs');
 await verifyPhotoBrowser(preview,listings);
}
...
console.log('::notice title=Verified photo performance candidate::'+JSON.stringify({candidate,preview,prior,priorHash,candidateHash:source(after)}));
