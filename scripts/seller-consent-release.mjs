...
if(!candidate){
 ...
 writeFileSync('wrangler.consent.json',JSON.stringify(config));
 let output;try{output=execFileSync('npx',['--yes','wrangler@4.129.0','versions','upload','--config','wrangler.consent.json'],{encoding:'utf8',stdio:['ignore','pipe','pipe'],maxBuffer:12e6});}catch{throw Error('Preview upload failed; CLI output withheld to protect configuration.');}
...
assert.equal(source(after),priorHash,'This UI fix must preserve the production Worker code');
...
console.log('::notice title=Verified two-checkbox candidate::'+JSON.stringify({candidate,preview,prior,priorHash,candidateHash:source(after)}));
