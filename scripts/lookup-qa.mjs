...
const wanted=['buyer_retry','targeted'].includes(round)?{seller:0,buyer:0}:round==='initial'?{seller:20,buyer:0}:round==='mixed'?{seller:10,buyer:10}:{seller:5,buyer:5};
...
if(round==='targeted'){
  for(const mode of ['seller','buyer'])for(let city=0;city<7;city++)catalogAll.push(...(await call(candidateAdapter,'/catalog?city='+city+'&mode='+mode)).rows);
  for(const target of JSON.parse(fs.readFileSync('scripts/lookup-qa-targeted.json','utf8'))){const c=catalogAll.find(c=>c.mode===target.mode&&hash(norm(c.address))===target.hash);assert(c,'Targeted prior case missing');cases.push({...c,id:target.id,lookup:target.lookup,group:'repair'});}
}
...
if(round==='final'){
  for(const target of JSON.parse(fs.readFileSync('scripts/lookup-qa-final-rechecks.json','utf8'))){const c=catalogAll.find(c=>c.mode===target.mode&&hash(norm(c.address))===target.hash);assert(c,'Targeted final recheck missing');cases.push({...c,id:target.id,lookup:target.lookup,group:'repair'});}
}
