...
if(!candidate){
 ...
 writeFileSync('wrangler.admin.json',JSON.stringify(config));
 let output;try{output=execFileSync('npx',['--yes','wrangler@4.129.0','versions','upload','--config','wrangler.admin.json'],{encoding:'utf8',stdio:['ignore','pipe','pipe'],maxBuffer:12e6});}catch{throw Error('Preview upload failed; CLI output withheld to protect configuration.');}
...
async function verify(base){
 ...
 for(const path of ['/api/admin/ops/counts','/api/admin/ops/agents','/api/admin/ops/analytics?from=2026-09-01&to=2026-09-23','/api/admin/ops/marketing'])assert.equal((await fetch(base+path,{signal:AbortSignal.timeout(20000)})).status,401,'Admin API is publicly accessible: '+path);
 let counts,ready;
 if(adminKey){
 ...
 console.log(JSON.stringify({stage:'verified',url:base,assets:assets.length,authenticatedChecks:!!adminKey,activeLeads:counts?.active,archivedLeads:counts?.archived,savedReportVerified:!!ready,publicAssetsUnchanged:true,adminAuthRequired:true}));
}
await verify(preview);
assert.equal(await active(),prior,'Production changed while preparing preview');
console.log('::notice title=Verified admin insights candidate::'+JSON.stringify({candidate,preview,prior,priorHash,candidateHash:source(after)}));
