...
function decorate(src){
  ...
  if(p.valuation?.available===true && vs.length>=3 && (p.report_type!=="THM Seller Price Perspective" || (p.seller?.evidence?.subjectMatched ?? p.seller?.evidence?.listingMatched))){
    ...
    const finalLow=Math.min(low,num(p.valuation?.low)||low),finalHigh=Math.max(high,num(p.valuation?.high)||high);
    p.valuation={...(p.valuation||{}),available:true,estimated_market_value:mv,market_value:mv,midpoint:mv,low:finalLow,high:finalHigh,likely_market_range:{low:finalLow,high:finalHigh},range_basis:'Evidence dispersion and confidence, preserving the evidence engine uncertainty allowance.'};
  }
