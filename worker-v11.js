...
async function querySoldComparableRows(baseFilters, env, top) {
  ...
  const cutoff = new Date(Date.now() - 400 * 864e5).toISOString();
  const recentFilters = [...baseFilters || [], `OriginalEntryTimestamp ge ${cutoff}`];
  let queryScope = "recent_entry_history";
  let pageResults = await Promise.all(Array.from({ length: 1 }, (_, page) => queryPropertiesDetailed(recentFilters, env, pageSize, null, page * pageSize)));
  if (!pageResults.some((result) => result.meta.status === 200)) {
    queryScope = "paged_local_history";
    pageResults = await Promise.all(Array.from({ length: 1 }, (_, page) => queryPropertiesDetailed(baseFilters, env, pageSize, "ModificationTimestamp desc,ListingKey desc", page * pageSize)));
  }
...
// worker-v11.js
var VERSION4 = "stage4-entry-window-vow-comparables-v93-20260903";
...
function groundReportNarrative(narrative, facts, valuation, comparables, policy = {}) {
  ...
  if (valuation.available) {
    grounded.market_read = `The ${range} evidence band is based on ${comparables.length} supplied sold matches. ${geography} The newest sold record is dated ${newest}.${policy.expandedWindow ? " An expanded 300-day evidence window was required." : ""} ${valuation.confidence === "Low" ? "Treat this as a broad screening signal, not a current value conclusion." : "Use the closest match as the starting point, then adjust for condition and micro-location."}`;
  } else {
    grounded.market_read = `No responsible sold-price band was produced from the supplied match set. ${geography}${policy.expandedWindow ? " The search was expanded to 300 days." : ""} Ask for a manual local comparable review before discussing value.`;
  }
...
function propertyReportEmail(address, agentData, report) {
  ...
  const expandedNote = policy.expandedWindow ? `<p style="margin:0 0 18px;padding:10px 12px;background:#fff6ee;color:#87511d;font:700 11px Arial,sans-serif;line-height:1.45">Evidence note: the search was expanded from 100 to 300 days.</p>` : "";
