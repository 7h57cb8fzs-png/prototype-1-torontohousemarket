...
async function buildPropertyReport(env,lead,property){
  ...
  const valuation={
    ...
    methodology:"Exact-subtype AMPRE/PropTx sales are checked within 100 days, expanded to 300 only when needed, prioritized by proximity, filtered to ±10% of the candidate-set median and used only when at least three qualified sales remain.",
  };
...
export function propertyReportEmail(address,agent,report){
  ...
  const compsHtml=comps.length?comps.map((c,i)=>visualComparable(c,i)).join(""):`<tr><td style="padding:16px;background:#f7f8fb;color:#626c80;font:600 13px Arial,sans-serif;line-height:1.5">No price conclusion was forced. The system needs at least three qualified exact-subtype sales before showing a range.</td></tr>`;
...
function reportSignal(facts,valuation,comps){
  if(!valuation.available||comps.length<3)return{title:"More evidence needed",note:"The AI found useful property facts but not enough qualified sold evidence to responsibly rate the asking price.",color:"#b66818",bg:"#fff6ea"};
