...
async function loadPropertyForReport(env, lead, requestId = null) {
  ...
  if (offerRecord) {
    ...
    vowBody.property.propertyHistory=summarizePropertyHistory(events);
  }
  try {
...
function mergeCurrentIdxWithVow(currentProperty, protectedProperty, subjectSource = "current_idx") {
  ...
  return {
    ...
    propertyHistory: protectedData.propertyHistory || null,
...
function propertyReportEmail(address, agentData, input, options = {}) {
  ...
  const label = /* @__PURE__ */ __name((t) => `<p style="margin:0 0 8px;color:#203b3c;font-family:Georgia,Times New Roman,serif;font-size:21px;line-height:1.3;font-weight:400;letter-spacing:0">${html(t)}</p>`, "label");
  const paragraph = /* @__PURE__ */ __name((t) => `<p style="margin:0 0 12px;font-size:16px;line-height:1.6;color:#53655e">${html(t)}</p>`, "paragraph");
  const bullets = /* @__PURE__ */ __name((values) => `<ul style="margin:0;padding-left:20px;color:#53655e;font-size:16px;line-height:1.65">${values.map((x) => `<li style="margin-bottom:8px">${html(x)}</li>`).join("")}</ul>`, "bullets");
  const section = /* @__PURE__ */ __name((name, body) => `<tr><td class="report-section" style="padding:22px 26px;border-bottom:1px solid #dce3dc">${label(name)}${body}</td></tr>`, "section");
...
async function resolveSellerSubject(address, profile, env, diagnostics = {}) {
  ...
  if (!candidates.size) {
    ...
    await runFilters(recovery.plans.map(p=>p.filter),300);
  }
  ...
  const allHistory = new Map(rows.map(r=>[r.ListingKey,r]));
  for (const r of records) allHistory.set(r.ListingKey,r);
  result._sellerHistory = [...allHistory.values()].map(r=>({...historyEvent(r),recordedAt:sellerListingTime(r)?new Date(sellerListingTime(r)).toISOString():null}));
...
async function loadSellerPropertyForReport(env, lead, requestId) {
  ...
  const property = { ...historical,
    ...
    propertyHistory: summarizePropertyHistory(raw?._sellerHistory || [], {complete:raw?._sellerHistoryComplete}),
...
function sellerReportEmail(address, report) {
  ...
  const section = /* @__PURE__ */ __name((number, title, body) => `<tr><td class="pad" style="padding:26px 30px;border-top:1px solid #dce3dc"><p style="${label}">${html(number)}</p><h2 style="margin:0 0 17px;font-family:Georgia,'Times New Roman',serif;font-weight:400;font-size:27px;line-height:1.25;color:#203b3c">${html(title)}</h2>${body}</td></tr>`, "section");
...
export {
  ...
  sellerLocalTrend,
  sellerParsedAddress,
  sellerHistoryParsedAddress,
  sellerHistoryMatches,
  sellerHomeKey,
  sellerQueryRows,
  sellerReportEmail,
  sellerSale,
