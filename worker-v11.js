...
function propertyReportEmail(address, agentData, report) {
  ...
  const avmCompVisual = avm.available && topComps.length ? `<table width="100%" cellpadding="0" cellspacing="0" border="0" style="margin:0 0 24px;background:#f7f8fb"><tr><td style="padding:18px"><p style="margin:0 0 10px;color:#3155f5;font:800 10px Arial,sans-serif;letter-spacing:1px">COMPARABLE CALCULATION</p>${topComps.map((c, i) => `<p style="margin:0;padding:9px 0;border-bottom:1px solid #e3e7ef;color:#4e586d;font:400 10px Arial,sans-serif;line-height:1.55"><strong style="color:#151b2b">${i + 1}. ${html(c.address || "MLS comparable")}</strong><br>${html(c.soldDate || "Date unavailable")} · ${html(cad(c.soldPrice) || "Price unavailable")} · ${c.distanceKm != null ? `${html(Number(c.distanceKm).toFixed(2))} km` : "distance unavailable"}<br>Similarity ${html(Math.round(Number(c.similarity) || 0))}% · recency ${html(Math.round(Number(c.recencyWeight || 0) * 100))}% · final weight ${html(c.finalWeightPct ?? "-")}%<br>Adjusted value ${html(cad(c.adjustedValue) || "-")} · market-time adjustment ${c.marketTimeAdjustment == null ? "not applied" : html(cad(c.marketTimeAdjustment))}<br>${Array.isArray(c.importantDifferences) && c.importantDifferences.length ? html(c.importantDifferences.join("; ")) : "No quantified material difference available."}</p>`).join("")}</td></tr></table>` : "";
  const evidenceVisual = `${avmVisual}${avmCompVisual}${ratingVisual}${priceGauge}${expandedNote}`;
