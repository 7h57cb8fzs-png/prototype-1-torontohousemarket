...
## Seller address and archive correction — 2026-09-14
Latest user direction supersedes upgrade features: remove upgrades from seller form, generated reports and confirmation emails. Keep stored legacy input for possible later restoration. Address first, then optional expected range and contact details.
- Dedicated public `/api/seller/address` verifies exact public listing addresses or exact geocoded street numbers/names/cities; building verification never claims a condo unit was verified. No VOW records exposed in public lookup.
- Private seller subject search prioritizes `ContractStatus eq 'Unavailable'` with exact street/number, checks exact unit/city locally, then falls back to status-unfiltered exact address queries. Older-than-ten-year specifications remain eligible. No old asking price becomes a sale price.
- Removed seller use of the generic count/tail-skip helper. Explicit newest-first queries and provider cursor pagination replace the unsupported assumption that the tail holds newer data. Current and unavailable inventory queried separately; full fields preserve historic specifications and sold fields. Buyer algorithm unchanged.
- Report shows sold-based suggested range plus separate active asking-price comparisons. Same community/type; same verified condo building exception and exact size preserved. Freeholds with missing floor area may use recorded bedrooms and lot frontage with a wider Low-confidence interval; no invented floor area.
- Removed upgrade estimates from emails, including legacy saved report rendering. Dormant upgrade helper retained for later work; no upgrade AI calls.
- Inspected three existing seller reports: 64 Ampezzo had no historical match; 112 Andrea Lane and 261 Howland recovered cancelled/closed history but insufficient comps. Thus historical access existed for some addresses; missing record retrieval is not automatically an access failure.
- Three focused automated scenarios passed (no live emails/leads). Live admin request returned HTTP 403 from this environment; new live valuations remain unverified. Do not claim three live properties passed. Existing admin read-only preview now includes recovered row counts and active competitors for diagnosis.
